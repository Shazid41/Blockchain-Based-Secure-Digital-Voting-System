...
export default function AvailableElectionsPage() {
  ...
  const elections = useMemo(() => rows.filter((election) => {
    const eligibility = eligibilityFor.get(election.id);
  ...
  return (
    <>
      ...
      <section className="container-page space-y-6 py-8">
        {error ? <AlertMessage type="error">Could not load live elections from Supabase. {error}</AlertMessage> : null}
        ...
        <div className="grid gap-4">
          {elections.length ? elections.map((election) => <ElectionCard key={election.id} election={election} eligibility={eligibilityFor.get(election.id)} />) : <EmptyState title="No elections match the filters" />}
        </div>
      </section>
    </>
  );
