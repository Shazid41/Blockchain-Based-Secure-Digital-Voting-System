...
import { listElections, saveElection } from '../../services/electionService.js';
...
export default function ManageElectionsPage() {
  ...
  useEffect(() => {
    Promise.all([listElections(), listRegions()])
      .then(([electionRows, regionRows]) => {
        setRows(electionRows);
  ...
  async function save(event) {
    ...
    try {
      const saved = await saveElection({ ...form, region_id: form.region_id || null });
  ...
  const electionRegionName = (election) => election.regions?.name ?? regions.find((region) => region.id === election.region_id)?.name ?? 'All regions';
    ...
    <>
      ...
      <section className="container-page space-y-6 py-8">
        <form className="card grid gap-4 p-5 lg:grid-cols-2" onSubmit={save}>
          ...
          <SelectInput id="electionRegion" label="Region" options={regions} value={form.region_id} onChange={(e) => setForm({ ...form, region_id: e.target.value })} />
        ...
        <div className="card grid gap-4 p-5 lg:grid-cols-4">
          ...
          <SelectInput id="electionFilterRegion" label="Region" options={regions} value={filters.region} onChange={(e) => setFilters({ ...filters, region: e.target.value })} />
